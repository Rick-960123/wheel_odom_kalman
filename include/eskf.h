...
public:
     ...
     * @return
     */
    bool Predict(const sensor_msgs::Imu::ConstPtr& msg, double dt);
     ...
     * @return
     */
    bool Correct(double& v, double& w, double& dt);
...
private:
    ...
    std::string robot_id;
    std::string imu_name;
    std::vector<double> V_T_i_b, V_R_acc_mea, V_R_gyr_mea, acc_bias, gyr_bias;
    Eigen::Matrix<double, 4, 4> T_i_b;
    Eigen::Matrix3d R_i_enc;
    Eigen::Vector3d t_i_enc;
    Eigen::Matrix<double, 3, 3> R_acc_mea, R_gyr_mea;
    double sigma_xg, sigma_nug, sigma_xa, sigma_nua, hz;
    double gravity;
